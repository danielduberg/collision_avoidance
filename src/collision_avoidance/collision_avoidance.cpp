...
bool CollisionAvoidance::avoidCollision(geometry_msgs::PoseStamped setpoint)
{
  ...
  if (control.twist.angular.z < yaw_converged_)
  {
    ...
    Eigen::Vector2d control_2d;  // TODO
...
void CollisionAvoidance::getObstacles(std::vector<Eigen::Vector2d>* obstacles)
{
  // TODO
  ...
  for (pcl::PointWithRange pr : range_image)
  {
    pcl::isFinite(pr);
  }
...
//     int index = ((int)std::round(std::atan2(point.y, point.x) * (polar_hist.size() / (2 * M_PI)))) %
//     polar_hist.size();
